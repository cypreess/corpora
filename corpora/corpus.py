...
class Corpus:
    ...
    def __init__(self, path, buffer_size=10485760):
    ...
    def get_by_idx(self, idx):
        ...
        if not (chunk_number == self.buffer_chunk and offset + head_len + text_len <= self.buffer_offset + self.buffer_size and offset >= self.buffer_offset):
            chunk = self.get_chunk(chunk_number)
            chunk.seek(offset,0)
            if head_len+text_len > self.buffer_size:
                self.buffer = chunk.read(head_len + text_len)
            else:
                self.buffer = chunk.read(self.buffer_size)
            self.buffer_offset = offset
            self.buffer_chunk = chunk_number

        head = self.buffer[offset-self.buffer_offset:offset-self.buffer_offset+head_len]
        text = self.buffer[offset-self.buffer_offset+head_len:offset-self.buffer_offset+head_len+text_len-2]    # -2 stripping two end-of-lines
